...
def main ():
    ...
    AQI(data,h)
    AQI_Feature_importance(data,h)
    AQI_Domain_Knowledge(data,h)
    AQI_without_Domain_Knowledge(data,h)

## Predincting for nxt day
    y=pd.read_csv('/home/gaurav/Desktop/IIITD/ML/project_k/untitled folder/AQI_prediction_add.csv')
    AQI_Future(data,y.AQI_predicted,h)
    AQI_Feature_importance_Future(data,y.AQI_predicted,h)
    AQI_Domain_Knowledge_Future(data,y.AQI_predicted,h)
    AQI_without_Domain_Knowledge_Future(data,y.AQI_predicted,h)
    # AQI_predicted
...
def BestParams(data):
    ...
    Hyper_params = np.array(
        [1, 0.011, 0.1, 0.001, 0.01, .3, .2, 0.6, .8, 0.001, 0.0001, 1, 2, 3, 4])

    Reg_model = Lasso()
    GSCgrid = gsc(estimator=Reg_model, param_grid=dict(alpha=Hyper_params))
    GSCgrid.fit(data, y)
    print('Hyper Parameter for Lasso:', GSCgrid.best_estimator_.alpha)
...
def AQI_Feature_importance(data,h):
...
#Predicting AQI using all features
def AQI_Domain_Knowledge(data,h):
    y=data.AQI
    # data=pd.get_dummies(data, columns=['month'], prefix = ['month'])
...
def AQI_without_Domain_Knowledge(data,h):
    y=data.AQI
    # data=pd.get_dummies(data, columns=['month'], prefix = ['month'])
    data=data.drop('AQI',axis=1)
...
#Predicting AQI using features from features importance graph
def AQI_Feature_importance_Future(data,y,h):
    tree_clf = ExtraTreesRegressor()
    # data=pd.get_dummies(data, columns=['month'], prefix = ['month'])
    tree_clf.fit(data, y)
    importances = tree_clf.feature_importances_
...
#Predicting AQI using all features
def AQI_Domain_Knowledge_Future(data,y,h):
    data=pd.get_dummies(data, columns=['month'], prefix = ['month'])
    x=data[[' _tempm',' _wdird',' _wspdm','year','Type_Industrial Area','month_10','month_11',]]
    X_train, X_val, y_train, y_val = train_test_split(x, y, test_size=0.2, random_state=1)
    regr = Lasso(alpha=h)
...
def AQI_without_Domain_Knowledge_Future(data,y,h):
    data=pd.get_dummies(data, columns=['month'], prefix = ['month'])
    data=data.drop('AQI',axis=1)
    data=data.drop('NO2',axis=1)
    data=data.drop('SO2',axis=1)
